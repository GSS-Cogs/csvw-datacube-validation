...
def get_function_map_from_config(config):
    ...
    for source in config.options.module_sources:
        ...
        if source.startswith("/") and "." not in source:
            ...
            function_map = _update_map_from_local_dir(function_map, source)
...
def _update_map_from_local_dir(function_map, source_dir):

    all_files = [x for x in os.listdir(source_dir) if x.endswith(".py")]

    for path in all_files:
        function_map = _update_map_from_file(function_map, source_dir + "/" + path)

    return function_map


def _update_map_from_file(function_map, path):
